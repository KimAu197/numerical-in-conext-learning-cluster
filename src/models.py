...
class TransformerModel(nn.Module): #
    def __init__(self, n_dims, n_positions, n_embd=128, n_layer=12, n_head=4, n_cluster=2, type = "gpt2"):
        # - **`n_dims`**：输入数据的特征维度。
        ...
        self._read_out = nn.Linear(n_embd, n_dims) #输出映射层 --> 标量
    ...
    def _combine(xs_b, ys_b):
        ...
        if ys_b.shape != xs_b.shape:
            ys_b_wide = torch.zeros(bsize, points, dim, device=xs_b.device)  # 形状为 [bsize, points, dim]
            ys_b = torch.argmax(ys_b, dim=-1)  # 形状变为 [bsize, points]
            ys_b = ys_b.long()
            ys_b_wide.scatter_(2, ys_b.unsqueeze(-1), 1)  # 使用        scatter_ 填充
    ...
    def forward(self, xs, ys, inds=None):
        ...
        result = prediction[:, ::2, :self.n_cluster][:, inds]
        # softmax_result = F.softmax(result, dim=-1)  # 形状为 [batch_size, num_inds, n_cluster]
        return result
